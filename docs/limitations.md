...
- Local `/api/state` GET is read-only (no auto-research). Mutations are POST. Default bind is `127.0.0.1:4317`. Remote bind (`0.0.0.0`) requires `SAT_API_TOKEN` plus `SAT_ALLOWED_ORIGINS`
