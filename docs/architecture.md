...
`getOperatingMode()` remaps `LIVE` → `PAPER`. `READ_ONLY` blocks paper execution. Execution plans set `canBroadcast: false`. `isLiveTradingAllowed()` is hard-false in V1. Default HTTP bind is loopback.
